      ...
      //  console.log(teamName);
        ...
        for(let j=0 ; j<allTrs.length-1 ; j++){
            ...
            if(allTds.length > 1){
                ...
                processLeaderboard(teamName , batsmanName , runs , balls , fours , sixes , strikRate);
            }
        }
    }
  //  console.log("#############################################");
...
function  processLeaderboard(teamName , batsmanName , runs , balls , fours , sixes , strikRate){
   runs = Number(runs);
   balls = Number(balls);
   fours = Number (fours);
   sixes = Number(sixes);
   for(let i = 0;i<processLeaderboard.length;i++){
       let lbObject = leaderboard[i];
       if(lbObject == teamName && lbObject.Batsman == batsmanName){
          lbObject.Runs += runs;
